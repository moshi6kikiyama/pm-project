<template>
  <div class="pt-[30px] text-white bg-[#171717] min-h-screen">
    ...
    <div class="max-w-xl mx-auto flex flex-col items-center space-y-[15px]">
      <img v-if="user?.avatar_url" class="w-[200px] h-[200px] rounded-full object-cover" :src="user?.avatar_url" alt="" />
      <img v-else class="w-[200px] h-[200px] rounded-full object-cover" src="https://t3.ftcdn.net/jpg/05/16/27/58/360_F_516275801_f3Fsp17x6HQK0xQgDQEELoTuERO4SsWV.jpg" alt="" />
...
const user = ref();
const follow = async () => {
  const res = serviceStore.api.follow(route.params.id as string, jwtData.id);
};
